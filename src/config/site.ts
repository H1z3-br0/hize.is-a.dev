...
export const projects: Project[] = [
  {
    ...
    text: {
      ru: {
        title: 'Изоморфизм решёток',
        summary: 'Определяет, задают ли два целочисленных базиса одну и ту же решётку.',
        ...
        approach: 'Алгоритм Плескена–Сувинье. Оба базиса LLL-редуцируются, короткие векторы перечисляются по Финке–Посту, кандидаты отсекаются по fingerprint — набору характеристик, инвариантных относительно изометрии, — и изометрия достраивается по одному вектору за раз с возвратом.',
      ...
      en: {
        ...
        approach: 'The Plesken–Souvignier algorithm. Both bases are LLL-reduced, short vectors are enumerated by Fincke–Pohst, candidates are pruned by a fingerprint of isometry-invariant characteristics, and the isometry is extended one vector at a time with backtracking.',
  ...
  },
  {
    ...
    text: {
      ru: {
        ...
        approach: 'TODO: устройство — что на Rust, что хранит состояние, как раздаются задания.',
        done: 'TODO: что уже работает и запускалось ли на реальном соревновании.',
      },
      en: {
        title: 'Lambda',
        ...
        approach: 'TODO: architecture — what is in Rust, where state lives, how tasks are served.',
        done: 'TODO: what already works, and whether it has run a real event.',
      },
    },
  },
  {
    id: 'ctf-infra',
    tools: ['Ansible', 'Proxmox', 'WireGuard', 'Nginx'],
    repo: null,
    text: {
      ru: {
        title: 'Инфраструктура для CTF',
        summary: 'Стенды и сеть под соревнования, разворачиваемые с нуля одной командой.',
        task: 'TODO: под что стенд — тренировки команды, школьные соревнования, что-то ещё.',
        approach: 'TODO: как связаны Proxmox, Ansible и WireGuard; что именно описано в плейбуках.',
        done: 'TODO: что развёрнуто и сколько людей этим пользуется.',
      },
      en: {
        title: 'CTF infrastructure',
        summary: 'Ranges and networking for competitions, deployable from scratch in one command.',
        task: 'TODO: what the range is for — team practice, school events, something else.',
        approach: 'TODO: how Proxmox, Ansible and WireGuard fit together; what the playbooks cover.',
        done: 'TODO: what is deployed and how many people use it.',
      },
    },
  },
];
...
export const stack: Profile[] = [
  ...
  },
  {
    ...
    note: { ru: 'классический ML и глубокое обучение', en: 'classical ML and deep learning' },
  },
  {
    id: 'devops',
...
export const ui: Record<Lang, UiStrings> = {
  ru: {
    ...
    latticeNoscript: 'Здесь интерактивная решётка — она требует JavaScript. Всё остальное на странице работает без него.',
  ...
  en: {
    ...
    latticeNoscript: 'An interactive lattice lives here; it needs JavaScript. Everything else on the page does not.',
...
export const origin = identity.domain ? `https://${identity.domain}` : 'http://localhost:4321';
